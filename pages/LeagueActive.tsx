    ...
    return (
        <div className="space-y-6 pb-32 animate-fade-in">
            ...
            <Modal
                ...
                body="Selecciona el nivel oficial para esta categoría:"
                icon={<Edit2 size={28}/>}
                iconColor="brand"
                size="sm"
                actions={[
                    { label: 'Cancelar', onClick: () => setEditingCategory(null), variant: 'secondary' },
                    { label: 'Guardar', onClick: handleUpdateCategory, variant: 'primary' },
                ]}
            >
                <div className="grid grid-cols-2 gap-2">
                    {TOURNAMENT_CATEGORIES.map(cat => (
                        <button key={cat} onClick={() => editingCategory && setEditingCategory({ ...editingCategory, name: cat })} className={`p-3 rounded-xl border text-xs font-bold transition-all ${editingCategory?.name === cat ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-slate-50 text-slate-600 border-slate-200 hover:border-indigo-200'}`}>{cat}</button>
                    ))}
                </div>
            </Modal>
            ...
            <Modal
                ...
                size="sm"
                actions={[
                    { label: 'Cancelar', onClick: () => setShowAddCategory(false), variant: 'secondary' },
                    { label: 'Crear', onClick: handleAddCategory, variant: 'primary' },
                ]}
            >
                <div className="grid grid-cols-2 gap-2">
                    {TOURNAMENT_CATEGORIES.map(cat => (
                        <button key={cat} onClick={() => setNewCategoryName(cat)} className={`p-3 rounded-xl border text-xs font-bold transition-all ${newCategoryName === cat ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-slate-50 text-slate-600 border-slate-200 hover:border-indigo-200'}`}>{cat}</button>
                    ))}
                </div>
            </Modal>
            ...
            <Modal
                ...
                title={editingPairId ? 'Editar Pareja' : 'Inscribir Pareja'}
                icon={<Users size={28}/>}
                iconColor="brand"
                size="md"
                actions={[
                    { label: 'Cancelar', onClick: () => setIsPairModalOpen(false), variant: 'secondary' },
                    { label: editingPairId ? 'Guardar Cambios' : (!p2 ? 'Añadir como Solo' : 'Confirmar Inscripción'), onClick: handleSavePair, variant: 'primary' },
                ]}
            >
                <PlayerSelector label="CAPITÁN (Jugador 1)" selectedId={p1} onSelect={setP1} otherSelectedId={p2} players={availablePlayers.concat(p1 ? [state.players.find(p=>p.id===p1)!].filter(Boolean) : [])} onAddPlayer={addPlayerToDB} formatName={formatPlayerName}/>
                ...
                <PlayerSelector label="JUGADOR 2" selectedId={p2} onSelect={setP2} otherSelectedId={p1} players={availablePlayers.concat(p2 ? [state.players.find(p=>p.id===p2)!].filter(Boolean) : [])} onAddPlayer={addPlayerToDB} formatName={formatPlayerName}/>
            </Modal>
            ...
            <Modal
                ...
                body="Esta acción eliminará a la pareja de la liga."
                icon={<Trash2 size={28}/>}
                iconColor="danger"
                actions={[
                    { label: 'Cancelar', onClick: () => setShowDeleteConfirm(null), variant: 'secondary' },
                    { label: 'Eliminar', onClick: handleDeleteConfirm, variant: 'danger' },
                ]}
            />
            ...
            <Modal
                ...
                iconColor="brand"
                actions={[
                    { label: 'Cancelar', onClick: () => setShowGenerateConfirm(false), variant: 'secondary' },
                    { label: 'Confirmar', onClick: handleConfirmGenerate, variant: 'primary' },
                ]}
            >
                <p className="text-slate-500 text-sm">Se generarán los grupos y partidos.<br/><strong>Método:</strong> {genMethod === 'elo-balanced' ? 'Equilibrado' : 'Mix'}. <strong>Formato:</strong> {doubleRound ? 'Ida y Vuelta' : 'Solo Ida'}.</p>
            </Modal>

            <Modal
                isOpen={showPlayoffWizard}
                ...
                size="md"
                actions={[
                    { label: 'Cancelar', onClick: () => setShowPlayoffWizard(false), variant: 'secondary' },
                    { label: 'Generar Cuadro', onClick: handleConfirmPlayoffs, variant: 'primary' },
                ]}
            >
                <div className="space-y-4 text-left">
                    <div>
                        <label className="text-xs font-bold text-slate-400 uppercase mb-2 block">Clasificados por Grupo</label>
                        <div className="grid grid-cols-4 gap-2">{[1, 2, 4, 8].map(num => (<button key={num} onClick={() => setPoQuota(num)} className={`py-3 rounded-xl font-bold border-2 transition-all ${poQuota === num ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-slate-500 border-slate-100 hover:border-slate-300'}`}>{num}</button>))}</div>
                    </div>
                    <div>
                        <label className="text-xs font-bold text-slate-400 uppercase mb-2 block">Tipo de Cruce</label>
                        <div className="grid grid-cols-2 gap-3">
                            <button onClick={() => setPoCross('crossed')} className={`p-3 rounded-xl border-2 text-left transition-all ${poCross === 'crossed' ? 'border-indigo-500 bg-indigo-50' : 'border-slate-100'}`}><div className="font-bold text-slate-800 text-sm">Cruzado</div><div className="text-xs text-slate-500">1º A vs 4º B</div></button>
                            <button onClick={() => setPoCross('internal')} className={`p-3 rounded-xl border-2 text-left transition-all ${poCross === 'internal' ? 'border-indigo-500 bg-indigo-50' : 'border-slate-100'}`}><div className="font-bold text-slate-800 text-sm">Interno</div><div className="text-xs text-slate-500">1º A vs 4º A</div></button>
                        </div>
                    </div>
                    <div>
                        <label className="text-xs font-bold text-slate-400 uppercase mb-2 block">Formato</label>
                        <div className="flex bg-slate-100 p-1 rounded-xl">
                            <button onClick={() => setPoFormat('single')} className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${poFormat === 'single' ? 'bg-white shadow text-indigo-600' : 'text-slate-500'}`}>Partido Único</button>
                            <button onClick={() => setPoFormat('double')} className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${poFormat === 'double' ? 'bg-white shadow text-indigo-600' : 'text-slate-500'}`}>Ida y Vuelta</button>
                        </div>
                    </div>
                </div>
            </Modal>
            ...
            <Modal
                ...
                icon={alertMessage?.type === 'error' ? <AlertTriangle size={28}/> : <CheckCircle size={28}/>}
            ...
            <Modal
                ...
                size="sm"
                actions={[
                    { label: 'Cancelar', onClick: () => setEditingMatchId(null), variant: 'secondary' },
                    { label: 'Guardar Resultado', onClick: handleSaveScore, variant: 'primary' },
                ]}
            >
                <div className="space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                        <div><label className="text-xs font-bold text-slate-400 uppercase block mb-1">Sets P1</label><input type="number" value={setsA} onChange={e => setSetsA(parseInt(e.target.value))} className="w-full bg-slate-50 border p-3 text-2xl font-black text-center rounded-xl outline-none focus:border-indigo-400"/></div>
                        <div><label className="text-xs font-bold text-slate-400 uppercase block mb-1">Sets P2</label><input type="number" value={setsB} onChange={e => setSetsB(parseInt(e.target.value))} className="w-full bg-slate-50 border p-3 text-2xl font-black text-center rounded-xl outline-none focus:border-indigo-400"/></div>
                    </div>
                    <input value={scoreText} onChange={e => setScoreText(e.target.value)} placeholder="6/4 7/5" className="w-full bg-slate-50 border p-3 font-bold rounded-xl text-center outline-none focus:border-indigo-400"/>
                </div>
            </Modal>
            ...
            <Modal
                ...
                title={selectedPairId ? getPairName(selectedPairId) : ''}
                body="Calendario Completo"
                icon={<Clock size={28}/>}
                iconColor="brand"
                size="md"
            >
