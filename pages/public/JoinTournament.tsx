        ...
        const init = async () => {
            ...
            if (clubId) {
                ...
                try {
                    ...
                    // public data fetch failed; page shows with defaults
                }
            }
        };
        init();
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col items-center p-0 sm:p-6">
            ...
            <Modal
                isOpen={!!alertMessage}
                onClose={closeAlert}
                title={alertMessage?.type === 'error' ? 'Atención' : 'Todo listo'}
                ...
                actions={[{ label: alertMessage?.type === 'error' ? 'Revisar' : 'Continuar', onClick: closeAlert, variant: alertMessage?.type === 'error' ? 'danger' : 'primary' }]}
            >
                {generatedLink && (
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 flex flex-col gap-2 text-left">
                        <p className="text-xs text-slate-500 font-bold uppercase">Enlace para tu compañero:</p>
