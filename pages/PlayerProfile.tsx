...
const PlayerProfile: React.FC = () => {
  ...
  // Find player in current state (Source of Truth for Profile Data)
  ...
  const stats = useMemo(() => {
      ...
      // Helper to process a tournament state (current or past)
      const processTournamentData = (tData: TournamentState, date: string) => {
          // Find pairs involving this player
          ...
          playerPairs.forEach(pair => {
               ...
               const partnerName = partner?.name || 'Desconocido';

               // Find matches involving this pair
               ...
               matches.forEach(m => {
                   ...
                   if (oppPair) {
                       const op1 = tData.players.find(p => p.id === oppPair.player1Id)?.name.split(' ')[0];
                       const op2 = tData.players.find(p => p.id === oppPair.player2Id)?.name.split(' ')[0];
                       oppNames = `${op1} & ${op2}`;
                   }
                   ...
                   result.matchHistory.push({
                       ...
                       date: date,
                       round: m.round,
                       roundLabel: m.round <= 4 ? `Fase Grupo (R${m.round})` : m.round === 5 ? 'Cuartos' : m.round === 6 ? 'Semis' : 'Final',
                       partner: partnerName.split(' ')[0], // Short name
                       opponent: oppNames,
      ...
      if (state.status !== 'setup') {
          processTournamentData(state, new Date().toISOString());
      }
      ...
      return result;
  }, [playerId, state, pastTournaments]);
  ...
  return (
    <div className="space-y-6 pb-20">
       ...
       <div className="flex items-center justify-between">
          <button onClick={() => navigate(-1)} className="p-2 bg-white border border-slate-200 rounded-full text-slate-600">
              <ArrowLeft size={20} />
          </button>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm relative overflow-hidden">
          <div className="absolute top-0 right-0 p-4">
               <button onClick={() => setIsEditing(true)} className="p-2 text-slate-400 hover:text-blue-600 bg-slate-50 rounded-lg">
                   <Edit2 size={20} />
               </button>
          </div>
          
          <div className="flex flex-col items-center text-center mb-6">
               <div className="w-20 h-20 bg-slate-100 rounded-full flex items-center justify-center text-slate-400 mb-4 border-4 border-slate-50 shadow-inner">
                   <User size={40} />
               </div>
               <h1 className="text-2xl font-black text-slate-900">{player.name}</h1>
               {player.nickname && <p className="text-lg text-slate-500 italic font-medium">"{player.nickname}"</p>}
               <div className="mt-2 flex flex-wrap justify-center gap-2">
                   {player.categories && player.categories.length > 0 ? player.categories.map(c => (
                       <span key={c} className="px-3 py-1 bg-slate-100 text-slate-600 rounded-full text-xs font-bold uppercase tracking-wider">
                           {c}
                       </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-slate-600 border-t border-slate-100 pt-4">
               <div className="flex items-center gap-3 p-2">
                   ...
                   <span>{player.phone || 'Sin teléfono'}</span>
               </div>
               <div className="flex items-center gap-3 p-2">
                   <Mail size={18} className="text-blue-500"/>
                   <span className="truncate">{player.email || 'Sin email'}</span>
               </div>
          </div>
      </div>

      {/* 2. MIDDLE SECTION: Statistics Dashboard */}
      {stats && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col items-center justify-center">
                   ...
                   <span className="text-2xl font-black text-slate-900">{stats.mainTitles}</span>
                   <span className="text-[10px] uppercase font-bold text-slate-400 text-center">Campeón</span>
              </div>

              <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col items-center justify-center">
                   <Medal size={24} className="text-blue-500 mb-2"/>
                   <span className="text-2xl font-black text-slate-900">{stats.consTitles}</span>
                   <span className="text-[10px] uppercase font-bold text-slate-400 text-center">Consolación</span>
              </div>
              
              {/* Total Played */}
              <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm col-span-2 md:col-span-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                      <div className="p-2 bg-slate-100 text-slate-500 rounded-lg"><Grid size={20}/></div>
      ...
      {/* 3. BOTTOM SECTION: Match History */}
      <div>
          <h3 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2">
              <Calendar size={20} className="text-slate-400"/> Historial de Partidos
          </h3>
          
          <div className="space-y-3">
              {stats?.matchHistory.length === 0 ? (
                  <div className="text-center py-8 text-slate-400 italic bg-slate-100 rounded-xl border border-slate-200">Sin partidos registrados</div>
              ) : (
                  stats?.matchHistory.map((match, idx) => (
                      <div key={`${match.id}-${idx}`} className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm flex justify-between items-center">
                          <div>
                              <div className="flex items-center gap-2 mb-1">
                                  <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded uppercase ${match.result === 'W' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                              ...
                              <div className="text-sm font-bold text-slate-800">
                                  <span className="text-slate-400 font-normal">con</span> {match.partner} <span className="text-slate-400 font-normal">vs</span> {match.opponent}
                              </div>
                              <div className="text-xs text-slate-500 mt-1">
                                  {match.roundLabel} {match.bracket ? `(${match.bracket === 'main' ? 'P' : 'C'})` : ''}
                              </div>
                          </div>
              ...
              <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl animate-slide-up max-h-[90vh] overflow-y-auto">
                  ...
                  <div className="space-y-4">
                      <div>
                          ...
                          <input value={editForm.name} onChange={e => setEditForm({...editForm, name: e.target.value})} className="w-full border border-slate-300 rounded-lg p-3 mt-1" />
                      </div>
                      <div className="grid grid-cols-2 gap-4">
                          <div>
                              <label className="text-xs font-bold text-slate-500 uppercase">Apodo</label>
                              <input value={editForm.nickname || ''} onChange={e => setEditForm({...editForm, nickname: e.target.value})} className="w-full border border-slate-300 rounded-lg p-3 mt-1" />
                          </div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {TOURNAMENT_CATEGORIES.map(cat => (
                                    <button
                                        ...
                                        className={`px-2 py-1 rounded text-xs font-bold border transition-colors ${editForm.categories?.includes(cat) ? 'bg-emerald-500 text-white border-emerald-500' : 'bg-white text-slate-500 border-slate-300'}`}
                                    >
                                        {cat}
                                    </button>
                                ))}
                      ...
                      <div>
                          ...
                          <input value={editForm.email || ''} onChange={e => setEditForm({...editForm, email: e.target.value})} className="w-full border border-slate-300 rounded-lg p-3 mt-1" />
                      </div>
                      <div>
                          <label className="text-xs font-bold text-slate-500 uppercase">Teléfono</label>
                          <input value={editForm.phone || ''} onChange={e => setEditForm({...editForm, phone: e.target.value})} className="w-full border border-slate-300 rounded-lg p-3 mt-1" />
                      </div>
                  </div>
