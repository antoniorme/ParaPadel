...
// --- SUB-COMPONENT: PAIR DETAIL (Extracted for stability) ---
...
export default function ActiveTournament() {
  ...
  const getNextStepText = (pairId: string, isWinner: boolean, phase: string, courtId: number, bracket: string) => {
      ...
      if (phase === 'qf') {
          ...
          if (bracket === 'consolation') return "A Semis Cons.";
      }
      if (phase === 'sf') {
           if (!isWinner) return "Eliminado";
  ...
  return (
    <div className="fixed inset-0 overflow-y-auto bg-slate-900 text-white">
        ...
        <div className="relative z-10 p-4 md:p-8 space-y-6 pb-32 max-w-[1600px] mx-auto">
            ...
            {/* SCORE MODAL */}
            {selectedMatchId && (
                <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-[100] flex items-center justify-center p-4 animate-fade-in" onClick={() => setSelectedMatchId(null)}>
                    <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl animate-scale-in relative text-slate-900" onClick={(e) => e.stopPropagation()}>
                        <button onClick={() => setSelectedMatchId(null)} className="absolute top-4 right-4 p-2 bg-slate-100 rounded-full text-slate-500 hover:bg-slate-200 hover:text-slate-800 transition-colors"><X size={20}/></button>
                        <div className="text-center mb-6">
                            <h3 className="text-3xl font-black text-slate-900 mb-1">PISTA {state.matches.find(m => m.id === selectedMatchId)?.courtId}</h3>
                            <div className="inline-block px-3 py-1 bg-slate-100 rounded-full text-slate-500 font-bold text-xs uppercase tracking-wider">Ronda {state.matches.find(m => m.id === selectedMatchId)?.round}</div>
                        </div>
                        <div className="flex items-center justify-between gap-4 mb-8">
                            <div className="flex-1 w-1/2">
                                ...
                                <p className="text-xs font-bold text-center text-slate-500 px-1 mt-2 h-8 flex items-center justify-center leading-tight line-clamp-2 overflow-hidden">{state.matches.find(m => m.id === selectedMatchId) ? getPairName(state.matches.find(m => m.id === selectedMatchId)!.pairAId) : 'P1'}</p>
                            </div>
                            ...
                            <div className="flex-1 w-1/2">
                                ...
                                <p className="text-xs font-bold text-center text-slate-500 px-1 mt-2 h-8 flex items-center justify-center leading-tight line-clamp-2 overflow-hidden">{state.matches.find(m => m.id === selectedMatchId) ? getPairName(state.matches.find(m => m.id === selectedMatchId)!.pairBId) : 'P2'}</p>
                            </div>
                        </div>
                        <div className="flex gap-3">
            ...
            {nextMatchInfo && (
                <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-[100] flex items-center justify-center p-4">
                    <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl animate-scale-in text-center text-slate-900">
                        <div className="w-16 h-16 bg-blue-100 rounded-full flex items-center justify-center mx-auto mb-4 text-blue-600"><Info size={32} /></div>
                        ...
                        <div className="space-y-3 mb-6 text-left">
                            <div className={`p-3 rounded-xl border ${nextMatchInfo.pairA.highlight ? 'bg-emerald-50 border-emerald-100' : 'bg-slate-50 border-slate-200'}`}>
                                <div className="flex justify-between items-center mb-1"><p className="text-xs font-bold text-slate-400 uppercase">Pareja 1</p>{nextMatchInfo.pairA.status === 'win' ? <CheckCircle size={14} className="text-emerald-500"/> : <XCircle size={14} className="text-slate-400"/>}</div>
                            ...
                            <div className={`p-3 rounded-xl border ${nextMatchInfo.pairB.highlight ? 'bg-emerald-50 border-emerald-100' : 'bg-slate-50 border-slate-200'}`}>
                                <div className="flex justify-between items-center mb-1"><p className="text-xs font-bold text-slate-400 uppercase">Pareja 2</p>{nextMatchInfo.pairB.status === 'win' ? <CheckCircle size={14} className="text-emerald-500"/> : <XCircle size={14} className="text-slate-400"/>}</div>
                        ...
                        <button onClick={handleCloseInfoModal} className="w-full py-4 bg-slate-900 text-white rounded-xl font-bold shadow-lg active:scale-95 transition-transform">Continuar y Siguiente Partido</button>
                    </div>
                </div>
            )}
            ...
            {alertMessage && (
                <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-[200] flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl animate-scale-in text-center text-slate-900">
                        <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 ${alertMessage.type === 'error' ? 'bg-rose-100 text-rose-600' : 'bg-amber-100 text-amber-600'}`}>{alertMessage.type === 'error' ? <X size={32} /> : <AlertTriangle size={32} />}</div>
